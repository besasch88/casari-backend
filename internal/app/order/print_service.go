...
func newPrintService(storage *gorm.DB, pubSubAgent *ceng_pubsub.PubSubAgent, repository orderRepositoryInterface) printServiceInterface {
	return printService{
		storage:     storage,
...
func (s printService) createOrderFromEvent(event ceng_pubsub.TableEventEntity) error {
	now := time.Now()
...
func (s printService) getOrder(ctx *gin.Context, input getOrderInputDto) (orderEntityWithChilds, error) {
...
func (s printService) updateOrder(ctx *gin.Context, input updateOrderInputDto) (orderEntityWithChilds, error) {
